...
#----------------------------------------------------------------------------------------------------------------
# Память
stats_value_y = {'iteration' : [], 'alpha' : [], 'Count_point' : [], 'Mean': [], 'Variance':[], 'STD' :[],
...
stats_value_x = {'iteration' : [], 'alpha' : [], 'Count_point' : [], 'Mean': [], 'Variance':[], 'STD' :[],
                 'Shapiro.stats' : [],         'Shapiro.p_value' : [], 
                 "DAgostino.stats":[],         "DAgostino.p_value":[],
                 'Uniform.observable' : [],    "Uniform.critical":[], 
                 "Exponential.observable":[],  "Exponential.critical":[]
...
def programm(path, file, step, alpha):
    ...
    for i in range(len(data_x)):
        if((i - 1)*step < 0):
            data_x[i] /= max(pref_sum_sqr[(i)*step],1)
        else:
            data_x[i] /= max(pref_sum_sqr[(i)*step] - pref_sum_sqr[(i - 1)*step],1)
...
#--------------------Проверки критериев по y---------------------------------------------------------------------------
    ...
    stats_value_y['Count_point'].append(len(data_y))
    stats_value_y['Mean'].append(statistics.mean(data_y))
    if(len(data_y) >= 2):
        stats_value_y['Variance'].append(statistics.variance(data_y))
        stats_value_y['STD'].append(statistics.stdev(data_y))
    else:
        stats_value_y['Variance'].append(None)
        stats_value_y['STD'].append(None)
...
#--------------------Проверки критериев по x---------------------------------------------------------------------------
    chi_squared_uniform, critical_value_uniform, f_uniform = test_uniform(data_x)
    chi_squared_exponential, critical_value_exponential, f_exponential = test_exponential(data_x)
    ...
    stats_value_x['Count_point'].append(len(data_x))
    stats_value_x['Mean'].append(statistics.mean(data_x))
    if(len(data_x) >= 2):
        stats_value_x['Variance'].append(statistics.variance(data_x))
        stats_value_x['STD'].append(statistics.stdev(data_x))
    else:
        stats_value_x['Variance'].append(None)
        stats_value_x['STD'].append(None)
...
f = open(f"Sowing\\Fractal_convecs\\square_x\\koch_curve_pore_{id_pole}.txt")
data = f.readlines()
f.close()
for i in data:
    pref_sum_sqr.append(float(i))
#----------------------------------------------Создание директории
path = ['Statistic\\log_concentration',
          f"Statistic\\log_concentration\\pole_{id_pole}",f"Statistic\\log_concentration\\pole_{id_pole}\\Points_{id_point}",
          f"Statistic\\log_concentration\\pole_{id_pole}\\Points_{id_point}\\exp_{id_exp}",
          ]
for i in path:
    if not os.path.isdir(i):
        os.mkdir(i)
#-------------------------------------------------Получение файлов и запуск программы
...
with pd.ExcelWriter(f"Statistic\\log_concentration\\pole_{id_pole}\\Points_{id_point}\\exp_{id_exp}\\iter_{category}.xlsx") as writer:  
